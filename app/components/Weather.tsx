...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col gap-0.5">
      <span className="text-[7px] text-cyan-400/30 tracking-[0.2em] uppercase">{label}</span>
...
function WeatherModal({ data }: { data: WeatherData }) {
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-end justify-between pb-3 border-b border-cyan-500/10">
        <div>
          <div className="flex items-end gap-2 mb-1">
            <span className="text-5xl font-bold text-white leading-none">{data.temp}°</span>
            <span className="text-lg text-cyan-400/60 mb-1">C</span>
            {data.feelsLike !== undefined && (
              <span className="text-[10px] text-cyan-400/40 mb-1.5">feels {data.feelsLike}°</span>
            )}
          </div>
          <div className="text-[10px] text-cyan-400/70 tracking-widest uppercase">{data.condition}</div>
        </div>
        {(data.tempMin !== undefined && data.tempMax !== undefined) && (
          <div className="text-right">
            <div className="text-[9px] text-cyan-400/30 tracking-widest mb-1">TODAY</div>
      ...
      {/* Stats grid */}
      <div className="grid grid-cols-3 gap-3 pb-3 border-b border-cyan-500/10">
      ...
      {data.hourly && data.hourly.length > 0 && (
        <div className="pb-3 border-b border-cyan-500/10">
          <div className="text-[8px] text-cyan-400/30 tracking-[0.2em] uppercase mb-3">Next 24 Hours</div>
          <div className="flex gap-2 overflow-x-auto pb-1">
            {data.hourly.map((h, i) => (
              <div key={i} className="flex flex-col items-center gap-1 shrink-0 min-w-[40px]">
                <span className="text-[8px] text-cyan-400/40 font-mono">{h.time}</span>
                <span className="text-base text-cyan-300">{h.icon}</span>
                <span className="text-[10px] text-white/70 font-mono">{h.temp}°</span>
                {h.rain > 0 && (
                  <span className="text-[7px] text-cyan-400/40">{h.rain}%</span>
                )}
              </div>
            ))}
          </div>
      ...
      {/* 7-day forecast */}
      <div>
        <div className="text-[8px] text-cyan-400/30 tracking-[0.2em] uppercase mb-3">7-Day Forecast</div>
        <div className="grid grid-cols-7 gap-1">
          {data.forecast.map(({ day, temp, icon }) => (
            <div key={day} className="flex flex-col items-center gap-1">
              <span className="text-[8px] text-cyan-400/35 tracking-widest">{day}</span>
              <span className="text-base text-cyan-300">{icon}</span>
      ...
      {(data.sunrise || data.sunset) && (
        <div className="flex items-center gap-6 pt-1 border-t border-cyan-500/10">
          <div className="flex items-center gap-2">
            <Sun size={12} className="text-amber-400/50" />
            <div>
              <div className="text-[7px] text-cyan-400/30 tracking-widest uppercase">Sunrise</div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="text-[7px] text-cyan-400/30 tracking-widest uppercase">Sunset</div>
        ...
        <div>
          <div className="flex items-start justify-between mb-1">
            <div className="flex items-end gap-2">
              ...
              <span className="text-sm text-cyan-400/60 mb-1">C</span>
            </div>
            {live && <span className="text-[8px] text-cyan-400/30 tracking-widest uppercase mt-1">LIVE</span>}
          </div>
          <div className="text-xs text-cyan-400 tracking-widest uppercase">{data.condition}</div>
          <div className="flex gap-4 mt-3 text-[10px] text-cyan-400/50">
            <span className="flex items-center gap-1"><Droplets size={11} className="text-cyan-400/60" />{data.humidity}%</span>
            <span className="flex items-center gap-1"><Wind size={11} className="text-cyan-400/60" />{data.wind}km/h</span>
            <span className="flex items-center gap-1"><Sun size={11} className="text-cyan-400/60" />UV {data.uv}</span>
          </div>
          <div className="flex justify-between mt-4 pt-3 border-t border-cyan-500/20">
            {data.forecast.map(({ day, temp, icon }, i) => (
              <div key={day} className={`flex flex-col items-center gap-1 ${i >= 5 ? "hidden 2xl:flex" : ""}`}>
                <span className="text-[9px] text-cyan-400/40 tracking-widest">{day}</span>
                <span className="text-cyan-300 text-base">{icon}</span>
